// GENERATED FILE — DO NOT EDIT.
// Source: design-system/tokens/*.json
// Rebuild: node design-system/build/build.mjs
// Verify:  node design-system/build/build.mjs --check
//
// React Native surface. Differences from the web emitter, all of them
// forced by the platform:
//   - dimensions are bare numbers, never `px` strings
//   - translucent colours are 8-digit hex, which RN parses natively;
//     rgba() strings are avoided because RN rejects them in some props
//   - easing is exported as cubic-bezier control points for
//     Easing.bezier(...) in Reanimated, not as a CSS keyword
//   - shadows would convert with shadowRadius = blur / 2, because iOS
//     shadowRadius is not CSS blur

export type Mode = "dark" | "light";

const THEMES = {
  "dark": {
    "color": {
      "surface": {
        "base": "#15141F",
        "raised": "#1E1D2E",
        "card": "#252438",
        "border": "#2E2C45",
        "inverse": "#FFFFFF",
        "notification": "#252438"
      },
      "text": {
        "primary": "#F0EFFE",
        "secondary": "#9B97B8",
        "muted": "#8D89A8",
        "onAccent": "#FFFFFF",
        "onNotification": "#FFFFFF"
      },
      "accent": {
        "default": "#7C6FCD",
        "strong": "#9B8FE0",
        "dim": "#3D3668"
      },
      "category": {
        "pain": {
          "mark": "#F2A69E",
          "ink": "#F2A69E",
          "icon": "pain",
          "label": "Pain",
          "onMark": "#1A1830"
        },
        "sleep": {
          "mark": "#9EC3F5",
          "ink": "#9EC3F5",
          "icon": "sleep",
          "label": "Sleep",
          "onMark": "#1A1830"
        },
        "energy": {
          "mark": "#F5D08A",
          "ink": "#F5D08A",
          "icon": "energy",
          "label": "Energy",
          "onMark": "#1A1830"
        },
        "mood": {
          "mark": "#A8D9B8",
          "ink": "#A8D9B8",
          "icon": "mood",
          "label": "Mood",
          "onMark": "#1A1830"
        },
        "meds": {
          "mark": "#C9B8F0",
          "ink": "#C9B8F0",
          "icon": "meds",
          "label": "Medication",
          "onMark": "#1A1830"
        }
      },
      "painScale": {
        "0": {
          "mark": "#A8D9B8",
          "ink": "#A8D9B8"
        },
        "1": {
          "mark": "#A8D9B8",
          "ink": "#A8D9B8"
        },
        "2": {
          "mark": "#B8DDA8",
          "ink": "#B8DDA8"
        },
        "3": {
          "mark": "#D9DA8A",
          "ink": "#D9DA8A"
        },
        "4": {
          "mark": "#F5D08A",
          "ink": "#F5D08A"
        },
        "5": {
          "mark": "#F5C070",
          "ink": "#F5C070"
        },
        "6": {
          "mark": "#F5A860",
          "ink": "#F5A860"
        },
        "7": {
          "mark": "#F2A09E",
          "ink": "#F2A09E"
        },
        "8": {
          "mark": "#E0748A",
          "ink": "#E0748A"
        },
        "9": {
          "mark": "#D4607F",
          "ink": "#D4607F"
        },
        "10": {
          "mark": "#BA4A79",
          "ink": "#BA4A79"
        }
      },
      "cta": {
        "from": "#6A5CB5",
        "to": "#7365C6",
        "label": "#FFFFFF"
      },
      "safety": {
        "ink": "#E86A5C",
        "mark": "#E05548",
        "surface": "#E0554822"
      },
      "state": {
        "disabled": "#2E2C45",
        "disabledText": "#5C5878",
        "selectedFill": "#7C6FCD22",
        "selectedEdge": "#7C6FCD44",
        "scrim": "#0000008C"
      }
    },
    "type": {
      "display": {
        "size": 44,
        "weight": 600,
        "lineHeight": 48,
        "tracking": -0.2
      },
      "heading": {
        "size": 28,
        "weight": 600,
        "lineHeight": 33.6,
        "tracking": 0
      },
      "title": {
        "size": 24,
        "weight": 600,
        "lineHeight": 36,
        "tracking": 0
      },
      "body": {
        "size": 15,
        "weight": 400,
        "lineHeight": 24,
        "tracking": 0
      },
      "rowLabel": {
        "size": 14,
        "weight": 500,
        "lineHeight": 21,
        "tracking": 0
      },
      "caption": {
        "size": 12,
        "weight": 600,
        "lineHeight": 18,
        "tracking": 0
      },
      "eyebrow": {
        "size": 11,
        "weight": 600,
        "lineHeight": 16.5,
        "tracking": 1.32
      },
      "buttonPrimary": {
        "size": 16,
        "weight": 600,
        "lineHeight": 24,
        "tracking": 0.16
      },
      "buttonSecondary": {
        "size": 14,
        "weight": 500,
        "lineHeight": 20,
        "tracking": 0
      },
      "insight": {
        "size": 15,
        "weight": 600,
        "lineHeight": 22,
        "tracking": 0
      }
    },
    "motion": {
      "press": {
        "duration": 120,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "toggle": {
        "duration": 150,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "control": {
        "duration": 200,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "indicator": {
        "duration": 250,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "screen": {
        "duration": 300,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "entrance": {
        "duration": 350,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "themeChange": {
        "duration": 400,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "draw": {
        "duration": 500,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "celebrate": {
        "duration": 600,
        "easing": [
          0.34,
          1.2,
          0.64,
          1
        ]
      },
      "ambient": {
        "duration": 2400,
        "easing": [
          0.42,
          0,
          0.58,
          1
        ]
      },
      "spinner": {
        "duration": 1800,
        "easing": [
          0,
          0,
          1,
          1
        ]
      }
    },
    "pattern": {
      "corridor": {
        "band": "#7C6FCD22",
        "edge": "#7C6FCD44",
        "label": "#9B97B8",
        "phaseChip": "#7C6FCD22",
        "phaseLabel": "#9B8FE0"
      },
      "rest": {
        "surface": "#15141F",
        "mark": "#2E2C45",
        "label": "#8D89A8"
      },
      "accumulation": {
        "counterFill": "#7C6FCD22",
        "counterEdge": "#7C6FCD44",
        "counterLabel": "#9B8FE0",
        "dotFilled": "#7C6FCD",
        "dotEmpty": "#2E2C45"
      },
      "lock": {
        "cardOpacity": 0.7,
        "badgeFill": "#2E2C45",
        "badgeGlyph": "#9B97B8",
        "badgeSize": 24,
        "glyphSize": 12,
        "label": "#9B97B8"
      },
      "historyFade": {
        "fadeFrom": "#1E1D2E0D",
        "fadeTo": "#1E1D2E",
        "pillFill": "#7C6FCD22",
        "pillEdge": "#7C6FCD44",
        "pillLabel": "#9B8FE0"
      },
      "insight": {
        "headline": "#F0EFFE",
        "headlineType": {
          "size": 15,
          "weight": 600,
          "lineHeight": 22,
          "tracking": 0
        },
        "trendUp": "#A8D9B8",
        "trendDown": "#F2A69E",
        "trendFlat": "#9B97B8",
        "chartLine": "#7C6FCD",
        "chartGrid": "#2E2C4588"
      },
      "fastPath": {
        "optionFill": "#252438",
        "optionEdge": "#2E2C45",
        "optionSelected": "#7C6FCD44",
        "minHeight": 88,
        "better": {
          "label": "Better",
          "mark": "#A8D9B8"
        },
        "same": {
          "label": "Same",
          "mark": "#9EC3F5"
        },
        "worse": {
          "label": "Worse",
          "mark": "#F2A69E"
        }
      },
      "share": {
        "cardFrom": "#3D3668",
        "cardTo": "#1E1D2E",
        "title": "#F0EFFE",
        "meta": "#9B97B8",
        "brandmark": "#F0EFFE55",
        "aspectRatio": 0.5625
      },
      "report": {
        "surface": "#1E1D2E",
        "heading": "#F0EFFE",
        "meta": "#9B97B8",
        "changeBlock": "#7C6FCD22",
        "divider": "#2E2C45"
      },
      "welcomeBack": {
        "surface": "#7C6FCD14",
        "message": "#F0EFFE"
      },
      "paywall": {
        "sheet": "#1E1D2E",
        "scrim": "#0000008C",
        "handle": "#2E2C45",
        "savingsFill": "#A8D9B833",
        "savingsLabel": "#A8D9B8"
      },
      "capture": {
        "field": "#1E1D2E",
        "edge": "#2E2C45",
        "placeholder": "#8D89A8",
        "actionIdle": "#2E2C45",
        "actionReady": "#7C6FCD"
      },
      "dayCard": {
        "from": "#332C55",
        "to": "#1E1D2E",
        "primary": "#F0EFFE",
        "secondary": "#9B97B8"
      },
      "toast": {
        "surface": "#252438",
        "label": "#FFFFFF",
        "dwell": 2400
      }
    }
  },
  "light": {
    "color": {
      "surface": {
        "base": "#F8F7FC",
        "raised": "#FFFFFF",
        "card": "#FFFFFF",
        "border": "#E4E1F5",
        "inverse": "#1A1830",
        "notification": "#252438"
      },
      "text": {
        "primary": "#1A1830",
        "secondary": "#6B6890",
        "muted": "#716AA9",
        "onAccent": "#FFFFFF",
        "onNotification": "#FFFFFF"
      },
      "accent": {
        "default": "#7C6FCD",
        "strong": "#6B5DBE",
        "dim": "#3D3668"
      },
      "category": {
        "pain": {
          "mark": "#F2A69E",
          "ink": "#AE4571",
          "icon": "pain",
          "label": "Pain",
          "onMark": "#1A1830"
        },
        "sleep": {
          "mark": "#9EC3F5",
          "ink": "#406CA9",
          "icon": "sleep",
          "label": "Sleep",
          "onMark": "#1A1830"
        },
        "energy": {
          "mark": "#F5D08A",
          "ink": "#876A34",
          "icon": "energy",
          "label": "Energy",
          "onMark": "#1A1830"
        },
        "mood": {
          "mark": "#A8D9B8",
          "ink": "#367B4D",
          "icon": "mood",
          "label": "Mood",
          "onMark": "#1A1830"
        },
        "meds": {
          "mark": "#C9B8F0",
          "ink": "#775BB9",
          "icon": "meds",
          "label": "Medication",
          "onMark": "#1A1830"
        }
      },
      "painScale": {
        "0": {
          "mark": "#A8D9B8",
          "ink": "#71927C"
        },
        "1": {
          "mark": "#A8D9B8",
          "ink": "#71927C"
        },
        "2": {
          "mark": "#B8DDA8",
          "ink": "#79926F"
        },
        "3": {
          "mark": "#D9DA8A",
          "ink": "#8D8E5A"
        },
        "4": {
          "mark": "#F5D08A",
          "ink": "#9F875A"
        },
        "5": {
          "mark": "#F5C070",
          "ink": "#A9844D"
        },
        "6": {
          "mark": "#F5A860",
          "ink": "#B87E48"
        },
        "7": {
          "mark": "#F2A09E",
          "ink": "#B97A79"
        },
        "8": {
          "mark": "#E0748A",
          "ink": "#D06C80"
        },
        "9": {
          "mark": "#D4607F",
          "ink": "#D4607F"
        },
        "10": {
          "mark": "#BA4A79",
          "ink": "#BA4A79"
        }
      },
      "cta": {
        "from": "#6A5CB5",
        "to": "#7365C6",
        "label": "#FFFFFF"
      },
      "safety": {
        "ink": "#C93D30",
        "mark": "#E05548",
        "surface": "#E0554822"
      },
      "state": {
        "disabled": "#E4E1F5",
        "disabledText": "#6B6890",
        "selectedFill": "#7C6FCD22",
        "selectedEdge": "#7C6FCD44",
        "scrim": "#0000008C"
      }
    },
    "type": {
      "display": {
        "size": 44,
        "weight": 600,
        "lineHeight": 48,
        "tracking": -0.2
      },
      "heading": {
        "size": 28,
        "weight": 600,
        "lineHeight": 33.6,
        "tracking": 0
      },
      "title": {
        "size": 24,
        "weight": 600,
        "lineHeight": 36,
        "tracking": 0
      },
      "body": {
        "size": 15,
        "weight": 400,
        "lineHeight": 24,
        "tracking": 0
      },
      "rowLabel": {
        "size": 14,
        "weight": 500,
        "lineHeight": 21,
        "tracking": 0
      },
      "caption": {
        "size": 12,
        "weight": 600,
        "lineHeight": 18,
        "tracking": 0
      },
      "eyebrow": {
        "size": 11,
        "weight": 600,
        "lineHeight": 16.5,
        "tracking": 1.32
      },
      "buttonPrimary": {
        "size": 16,
        "weight": 600,
        "lineHeight": 24,
        "tracking": 0.16
      },
      "buttonSecondary": {
        "size": 14,
        "weight": 500,
        "lineHeight": 20,
        "tracking": 0
      },
      "insight": {
        "size": 15,
        "weight": 600,
        "lineHeight": 22,
        "tracking": 0
      }
    },
    "motion": {
      "press": {
        "duration": 120,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "toggle": {
        "duration": 150,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "control": {
        "duration": 200,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "indicator": {
        "duration": 250,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "screen": {
        "duration": 300,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "entrance": {
        "duration": 350,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "themeChange": {
        "duration": 400,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "draw": {
        "duration": 500,
        "easing": [
          0,
          0,
          0.58,
          1
        ]
      },
      "celebrate": {
        "duration": 600,
        "easing": [
          0.34,
          1.2,
          0.64,
          1
        ]
      },
      "ambient": {
        "duration": 2400,
        "easing": [
          0.42,
          0,
          0.58,
          1
        ]
      },
      "spinner": {
        "duration": 1800,
        "easing": [
          0,
          0,
          1,
          1
        ]
      }
    },
    "pattern": {
      "corridor": {
        "band": "#7C6FCD22",
        "edge": "#7C6FCD44",
        "label": "#6B6890",
        "phaseChip": "#7C6FCD22",
        "phaseLabel": "#6B5DBE"
      },
      "rest": {
        "surface": "#F8F7FC",
        "mark": "#E4E1F5",
        "label": "#716AA9"
      },
      "accumulation": {
        "counterFill": "#7C6FCD22",
        "counterEdge": "#7C6FCD44",
        "counterLabel": "#6B5DBE",
        "dotFilled": "#7C6FCD",
        "dotEmpty": "#E4E1F5"
      },
      "lock": {
        "cardOpacity": 0.7,
        "badgeFill": "#E4E1F5",
        "badgeGlyph": "#6B6890",
        "badgeSize": 24,
        "glyphSize": 12,
        "label": "#6B6890"
      },
      "historyFade": {
        "fadeFrom": "#FFFFFF0D",
        "fadeTo": "#FFFFFF",
        "pillFill": "#7C6FCD22",
        "pillEdge": "#7C6FCD44",
        "pillLabel": "#6B5DBE"
      },
      "insight": {
        "headline": "#1A1830",
        "headlineType": {
          "size": 15,
          "weight": 600,
          "lineHeight": 22,
          "tracking": 0
        },
        "trendUp": "#A8D9B8",
        "trendDown": "#F2A69E",
        "trendFlat": "#6B6890",
        "chartLine": "#7C6FCD",
        "chartGrid": "#E4E1F588"
      },
      "fastPath": {
        "optionFill": "#FFFFFF",
        "optionEdge": "#E4E1F5",
        "optionSelected": "#7C6FCD44",
        "minHeight": 88,
        "better": {
          "label": "Better",
          "mark": "#A8D9B8"
        },
        "same": {
          "label": "Same",
          "mark": "#9EC3F5"
        },
        "worse": {
          "label": "Worse",
          "mark": "#F2A69E"
        }
      },
      "share": {
        "cardFrom": "#3D3668",
        "cardTo": "#FFFFFF",
        "title": "#1A1830",
        "meta": "#6B6890",
        "brandmark": "#1A183055",
        "aspectRatio": 0.5625
      },
      "report": {
        "surface": "#FFFFFF",
        "heading": "#1A1830",
        "meta": "#6B6890",
        "changeBlock": "#7C6FCD22",
        "divider": "#E4E1F5"
      },
      "welcomeBack": {
        "surface": "#7C6FCD14",
        "message": "#1A1830"
      },
      "paywall": {
        "sheet": "#FFFFFF",
        "scrim": "#0000008C",
        "handle": "#E4E1F5",
        "savingsFill": "#A8D9B833",
        "savingsLabel": "#A8D9B8"
      },
      "capture": {
        "field": "#FFFFFF",
        "edge": "#E4E1F5",
        "placeholder": "#716AA9",
        "actionIdle": "#E4E1F5",
        "actionReady": "#7C6FCD"
      },
      "dayCard": {
        "from": "#F0EDFB",
        "to": "#FFFFFF",
        "primary": "#1A1830",
        "secondary": "#6B6890"
      },
      "toast": {
        "surface": "#252438",
        "label": "#FFFFFF",
        "dwell": 2400
      }
    }
  }
} as const;

export const scale = {
  "space": {
    "0": 0,
    "1": 4,
    "2": 8,
    "3": 12,
    "4": 16,
    "5": 20,
    "6": 24,
    "8": 32,
    "10": 40,
    "12": 48
  },
  "radius": {
    "xxs": 2,
    "xs": 3,
    "sm": 8,
    "md": 12,
    "lg": 16,
    "xl": 20,
    "2xl": 24,
    "shell": 44,
    "full": 9999
  },
  "size": {
    "tap": 44,
    "control": 40,
    "iconTile": 34,
    "badge": 24,
    "glyph": 12,
    "dot": 6,
    "dotActive": 16,
    "sliderThumb": 24,
    "buttonPrimary": 56,
    "buttonSecondary": 48,
    "rowStandard": 52,
    "hairline": 1,
    "hairlineThick": 2,
    "shellWidth": 390,
    "shellHeight": 844
  },
  "z": {
    "base": 0,
    "raised": 10,
    "overlay": 40,
    "feature": 45,
    "sheet": 50,
    "toast": 60,
    "dev": 100
  },
  "duration": {
    "instant": 0,
    "micro": 120,
    "quick": 150,
    "compact": 200,
    "medium": 250,
    "screen": 300,
    "entrance": 350,
    "theme": 400,
    "draw": 500,
    "celebrate": 600,
    "dwell": 2400,
    "breathe": 2400,
    "spin": 1800
  },
  "easing": {
    "out": [
      0,
      0,
      0.58,
      1
    ],
    "in": [
      0.42,
      0,
      1,
      1
    ],
    "inOut": [
      0.42,
      0,
      0.58,
      1
    ],
    "linear": [
      0,
      0,
      1,
      1
    ],
    "gentle": [
      0.34,
      1.2,
      0.64,
      1
    ]
  },
  "stagger": {
    "step": 60,
    "max": 5
  },
  "font": {
    "family": [
      "Inter",
      "ui-sans-serif",
      "system-ui",
      "sans-serif"
    ],
    "weight": {
      "regular": 400,
      "medium": 500,
      "semibold": 600,
      "bold": 700
    },
    "size": {
      "3xs": 10,
      "2xs": 11,
      "xs": 12,
      "sm": 13,
      "base": 14,
      "md": 15,
      "lg": 16,
      "xl": 18,
      "2xl": 24,
      "3xl": 28,
      "display": 44
    },
    "tracking": {
      "tight": -0.2,
      "normal": 0,
      "wide": 0.16,
      "caps": 1.32
    }
  }
} as const;

export function theme(mode: Mode) {
  return THEMES[mode];
}

/**
 * Colours for a 0-10 pain score. `mark` fills (slider track), `ink` writes
 * (the numeral) — see color.painScale in the token source.
 *
 * Clamps and rounds, so a fractional or out-of-range score cannot produce an
 * undefined colour that renders as nothing.
 */
export function painStep(mode: Mode, score: number) {
  const i = Math.max(0, Math.min(10, Math.round(score)));
  return THEMES[mode].color.painScale[String(i) as keyof Theme["color"]["painScale"]];
}

export type Theme = (typeof THEMES)["dark"];
export type ColorRole = keyof Theme["color"];

/**
 * AnklePath-shaped palette, generated from the token source.
 *
 * A MIGRATION SHIM with a deliberate end date. Ported screens read
 * palette.text, and this lets them keep doing so while the values beneath
 * become Recovery Companion's. New code should use theme(mode) instead:
 * this shape is FLAT, so it cannot express mark-vs-ink, and that is the one
 * distinction this system most needs components to make.
 *
 * OMITTED ON PURPOSE: teal, tealDark, blue, blueDark, red, green, amber, purple, successSoft, warningSoft, dangerSoft, infoSoft.
 * These carry meaning rather than structure, and are absent so that every
 * site using one FAILS TO COMPILE and gets re-domained by hand. See
 * build/emit-native-palette.mjs for why each one is unsafe to map.
 */
export type RcPalette = {
  mode: Mode;
  background: string;
  backgroundSoft: string;
  surface: string;
  surfaceRaised: string;
  surfaceMuted: string;
  text: string;
  textMuted: string;
  textSubtle: string;
  border: string;
  borderSoft: string;
  white: string;
};

const RC_PALETTES = {
  "dark": {
    "mode": "dark",
    "background": "#15141F",
    "backgroundSoft": "#15141F",
    "surface": "#1E1D2E",
    "surfaceRaised": "#252438",
    "surfaceMuted": "#2E2C45",
    "text": "#F0EFFE",
    "textMuted": "#9B97B8",
    "textSubtle": "#8D89A8",
    "border": "#2E2C45",
    "borderSoft": "#2E2C45",
    "white": "#FFFFFF"
  },
  "light": {
    "mode": "light",
    "background": "#F8F7FC",
    "backgroundSoft": "#F8F7FC",
    "surface": "#FFFFFF",
    "surfaceRaised": "#FFFFFF",
    "surfaceMuted": "#E4E1F5",
    "text": "#1A1830",
    "textMuted": "#6B6890",
    "textSubtle": "#716AA9",
    "border": "#E4E1F5",
    "borderSoft": "#E4E1F5",
    "white": "#FFFFFF"
  }
} as const;

export function appPalette(mode: Mode): RcPalette {
  return RC_PALETTES[mode];
}

/** AnklePath keys Recovery Companion deliberately does not provide. */
export const RC_UNMAPPED_KEYS = ["teal","tealDark","blue","blueDark","red","green","amber","purple","successSoft","warningSoft","dangerSoft","infoSoft"] as const;
