...
### Correction — §11 was corrupted by a `# Recovery Companion — Design Criteria
...
progress screen. Verified as exactly that and nothing else.
 ...
 in a replacement string, and is repaired

Commit `f422bac` duplicated **27,094 characters** of this document into the
middle of itself. The mechanism: the §11 text being inserted contained the
literal sequence `` `# Recovery Companion — Design Criteria
...
progress screen. Verified as exactly that and nothing else.
 ...
 `` while describing the `$literals` key, and in
JavaScript's `String.replace` a dollar followed by a backtick means *"insert
everything before the match"*. So the whole document up to the anchor was pasted
in, and the sentence it interrupted resumed sixty lines later with a stray
" prefix meant the lookup always missed."

This is the second time that exact bug has appeared here — it also truncated
`redomain.mjs`, where it was caught immediately because the file stopped running.
In a Markdown document nothing stopped running, so it survived several commits
until the duplicated headings were noticed while reading §11 for something else.

Repaired: the duplicate block removed, the broken sentence restored. The lesson
is not "be careful with `# Recovery Companion — Design Criteria
...
" — it is that a document has no test, so the only
thing standing between it and silent corruption is somebody reading it.
